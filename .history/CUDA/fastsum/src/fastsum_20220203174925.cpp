...
void fastsum_precompute_source_nodes(fastsum_plan *ths)
{
  ...
  ths->source_node_loc.data = ths->x; // FIXME we need tyo take factor -1?
  //                                                                                             Sector, kernel, OSF, dimensions
  ths->gpuNUFFTOpSrc = ths->factory.createGpuNUFFTOperator(ths->source_node_loc, ths->density_compArray, ths->sensArray, 3, 8, 2, ths->imgDims);
