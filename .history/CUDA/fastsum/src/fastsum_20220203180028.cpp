...
void fastsum_trafo(fastsum_plan *ths)
{
  ...
  ths->gpuNUFFTOpSrc->performGpuNUFFTAdj(ths->src_data, src_adj_op);
#ifdef MEASURE_TIME
  t1 = getticks();
  ...
  #pragma omp parallel for default(shared) private(k)
#endif
  DType2 *adj_op_data = (DType2*)ths->src_adj_op.data; 
  for (k = 0; k < ths->mv2.N_total; k++)
    adj_op_data[k] = ths->b[k] * adj_op_data[k];
#ifdef MEASURE_TIME
  t1 = getticks();
...
ths->gpuNUFFTOp->performForwardGpuNUFFT(ths->src_adj_op, ths->target_op);
#ifdef MEASURE_TIME
  t1 = getticks();
