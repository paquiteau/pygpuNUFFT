...
void fastsum_trafo(fastsum_plan *ths)
{
  ...
  #pragma omp parallel for default(shared) private(k)
#endif
  std::complex<DType2 *adj_op_data = reinterpret_cast<std::complex<DType>(&)[0]>(adj_op_data[k])
  (DType2*)ths->src_adj_op.data; 
  for (k = 0; k < ths->N_total; k++)
    adj_op_data[k] = reinterpret_cast<std::complex<DType>(&)[0]>(ths->b[k]) * reinterpret_cast<std::complex<DType>(&)[0]>(adj_op_data[k]);
#ifdef MEASURE_TIME
  t1 = getticks();
