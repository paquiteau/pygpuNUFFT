...
C regkern(kernel_fs k, R xx, int p, const R *param, R a, R b)
{
  int r;
  C sum = K(0.0);
 ...
 *  (used in 1D)
 */
static C regkern1(kernel_fs k, R xx, int p, const R *param, R a, R b)
{
  int r;
  C sum = K(0.0);
...
static inline C calc_SearchBox(int d, R *y, R *x, C *alpha, int start,
    int end_lt, const C *Add, const int Ad, int p, R a, const kernel_fs k,
...
static C SearchTree(const int d, const int t, const R *x, const C *alpha,
    const R *xmin, const R *xmax, const int N, const kernel_fs k, const R *param,
...
static void fastsum_precompute_kernel(fastsum_plan *ths)
{
  ...
  NFFT(fftshift_complex)(&reinterpret_cast<DType(&)[2]>(*ths->b), (int)(ths->d), N);
...
void fastsum_init_guru_kernel(fastsum_plan *ths, int d, kernel_fs k, R *param,
...
#endif
  // gpuNUFFT INIT
  ths->sensArray.data = NULL; // We dont use sens for fastsum
  ths->density_compArray.data = NULL;  // We dont use density compensation for fastsum
...
void fastsum_init_guru_source_nodes(fastsum_plan *ths, int N_total, int nn_oversampled, int m)
{
  ...
  ths->x2 = (R *) NFFT(malloc)((size_t)(ths->d * N_total) * (sizeof(R)));
...
void fastsum_init_guru_target_nodes(fastsum_plan *ths, int M_total, int nn_oversampled, int m)
{
  ...
  ths->y2 = (R *) NFFT(malloc)((size_t)(ths->d * M_total) * (sizeof(R)));
...
void fastsum_init_guru(fastsum_plan *ths, int d, int N_total, int M_total,
    kernel_fs k, R *param, unsigned flags, int nn, int m, int p, R eps_I, R eps_B)
{
...
void fastsum_precompute_source_nodes(fastsum_plan *ths)
{
  ...
  t0 = getticks();
#endif
  /*for (int j = 0; j < ths->M_total; j++)
   for (int t = 0; t < ths->d; t++)
      ths->x2[ths->d * j + t] = -ths->x[ths->d * j + t]; */
...
/*for (int j = 0; j < ths->M_total; j++)
   for (int t = 0; t < ths->d; t++)
      ths->y[ths->d * j + t] = -ths->y[ths->d * j + t]; /* note the factor -1 for conjugated transform instead of standard*/
ths->target_node_loc.data = ths->y2; // FIXME we need tyo take factor -1?
...
void fastsum_trafo(fastsum_plan *ths)
{
  ...
  R scale_factor = SQRT(pow(2, ths->d))*0.948*SQRT(ths->imgDims.count()); //0.948 is weird scale factor between NFFT and 
  for (k = 0; k < ths->imgDims.count(); k++)
    adj_op_data[k] =  adj_op_data[k] * scale_factor;
#ifdef MEASURE_TIME
  t1 = getticks();
...
ths->target_op.dim.length = ths->M_total;
