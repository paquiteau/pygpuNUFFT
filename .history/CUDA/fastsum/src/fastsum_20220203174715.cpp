...
void fastsum_precompute_target_nodes(fastsum_plan *ths)
{
  ...
  t0 = getticks();
#endif
ths->target_node_loc = ths->y; // FIXME we need tyo take factor -1?
//                                                                                             Sector, kernel, OSF, dimensions
gpuNUFFTOpTgt = ths->factory.factory.createGpuNUFFTOperator(ths->target_node_loc, ths->density_compArray, ths->sensArray, 3, 8, 2, ths->imgDims);
