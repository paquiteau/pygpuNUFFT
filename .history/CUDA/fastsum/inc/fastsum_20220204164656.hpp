...
typedef C (*kernel_fs)(R , int , const R *);
...
typedef struct fastsum_plan_
{
  ...
  R *x, *x2;                            /**< source knots in d-ball with radius 1/4-eps_b/2 */
  R *y, *y2;                            /**< target knots in d-ball with radius 1/4-eps_b/2 */

  kernel_fs k;  /**< kernel_fs function    */
 ...
 * \param kernel_fs The kernel function.
 ...
 *
 */
void fastsum_init_guru(fastsum_plan *ths, int d, int N_total, int M_total, kernel_fs k, R *param, unsigned flags, int nn, int m, int p, R eps_I, R eps_B);
