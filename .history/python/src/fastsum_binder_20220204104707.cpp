...
py::array_t<R> FastSumOperator::sum(py::array_t<R> points, py::array_t<R> points2, bool direct)
{
  ...
  py::array_t<C> out_result({128, 128, 128});
  C *out_data = (C *) out_result.request().ptr;
  for (j=0; j<my_fastsum_plan.imgDims.count(); ++j)
  {
    out_data[j] = *reinterpret_cast<std::complex<DType>(&)[0]C>(&my_fastsum_plan.src_adj_op.data[j]);
  }
  return out_result;
  /** copy result 
